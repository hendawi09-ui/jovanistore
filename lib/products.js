export const cats = [
  ...
  { id: "sale", label: "عروض وتخفيضات", color: "var(--red)" },
];
...
// بيوحّد أشكال الألف والتاء المربوطة والياء، ويشيل التشكيل وعلامات الترقيم،
// عشان "احمر" تلاقي "أحمر" و"قميص، كتان" تلاقي "قميص كتان".
export function normalizeAr(text) {
  return String(text || "")
    ...
    .replace(/ة/g, "ه")
    .replace(/[ؤئ]/g, "ء")
    .replace(/[^\p{L}\p{N}\s]/gu, " ")        // أي رمز أو ترقيم يبقى مسافة
    .replace(/\s+/g, " ")
    .trim();
}

// بيرجع true لو كل كلمات البحث موجودة في النص (أي عدد كلمات)
