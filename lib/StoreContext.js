...
export function StoreProvider({ children }) {
  ...
  const [productsLoaded, setProductsLoaded] = useState(false); // بنعرف بيها إن المنتجات وصلت فعلًا
  const [storeMode, setStoreMode] = useState("all"); // all | women | men — بيتظبط من لوحة التحكم
  ...
  // وضع عرض المتجر (الكل / نسائي فقط / رجالي فقط)
  const refreshStoreMode = useCallback(async () => {
    try {
      // cache:"no-store" بيمنع المتصفح إنه يرجّع نسخة قديمة محفوظة عنده
      const res = await fetch("/api/store-mode", { cache: "no-store" });
      if (res.ok) {
        const d = await res.json();
        if (d?.mode) setStoreMode(d.mode);
      }
    } catch { /* لو فشل، بيفضل على "الكل" وهو الوضع الآمن */ }
  }, []);
  ...
  useEffect(() => {
    ...
    // المنتجات ووضع العرض بيتحمّلوا مع بعض — عشان ما يظهرش منتج مفروض يكون مخفي
    // للحظة قبل ما الوضع يوصل
    Promise.all([refreshProducts(), refreshStoreMode(), refreshOrders()]).finally(() => {
      setProductsLoaded(true);
      setReady(true);
    });
  }, [refreshProducts, refreshStoreMode, refreshOrders]);
  ...
  // تنظيف تلقائي: أي منتج اتحذف نهائيًا من المتجر بيتشال من المفضلة والسلة،
  // عشان ما يفضلش عنصر مكسور مالوش صفحة. بيشتغل بعد ما المنتجات توصل بس.
  useEffect(() => {
    if (!ready || !productsLoaded || products.length === 0) return;
    const alive = new Set(products.map((p) => String(p.id)));

    setFavorites((f) => {
      const next = {};
      let removed = false;
      for (const [key, entry] of Object.entries(f)) {
        if (alive.has(String(entry.id))) next[key] = entry;
        else removed = true;
      }
      return removed ? next : f;
    });
    ...
    setCart((c) => {
      ...
      return removed ? next : c;
    });
  }, [ready, productsLoaded, products]);
  ...
  // بتشيل المنتج من السلة بكل ألوانه ومقاساته — بيستخدمها زرار السلة في البطاقة
  const removeProductFromCart = useCallback((id) => {
    setCart((c) => {
      const next = {};
      for (const [key, entry] of Object.entries(c)) {
        if (entry.id != id) next[key] = entry;
      }
      return next;
    });
  }, []);
  ...
  // ---- المفضلة ----
  // بيرجع true لو المنتج موجود في السلة بأي لون/مقاس — بنستخدمها في بطاقة المنتج
  const isInCart = useCallback(
    (id) => Object.values(cart).some((entry) => entry.id == id && entry.qty > 0),
    [cart]
  );
  ...
  // تغيير كلمة السر (بيتحقق من الحالية الأول لو الحساب عنده واحدة)
